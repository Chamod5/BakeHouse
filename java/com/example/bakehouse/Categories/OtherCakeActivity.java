...
public class OtherCakeActivity extends AppCompatActivity {
    ...
    private static final String TAG = "OtherCakeActivity";
    private RecyclerView recyclerView;
    private OtherCakeAdapter cakeAdapter;
    private ArrayList<OtherCakeModel> otherCakeList;
    private ProgressBar progressBar;

    private static final String URL = DbLink.BASE_URL + "get_other_cakes.php";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_other_cake);
    ...
    private void setupRecyclerView() {
        ...
        otherCakeList = new ArrayList<>();
        cakeAdapter = new OtherCakeAdapter(this, otherCakeList);
        recyclerView.setAdapter(cakeAdapter);
    }

    private void loadOtherCakes() {
                    ...
                    try {
                        ...
                        if (response.getString("status").equals("success")) {
                            ...
                            if (cakesArray.length() == 0) {
                                Toast.makeText(this, "No cakes available", Toast.LENGTH_SHORT).show();
                                return;
                            }

                            otherCakeList.clear(); // Clear existing data

                            for (int i = 0; i < cakesArray.length(); i++) {
                                JSONObject cakeObj = cakesArray.getJSONObject(i);
                                OtherCakeModel cake = new OtherCakeModel();
                                ...
                                otherCakeList.add(cake);
                            }

                            cakeAdapter.notifyDataSetChanged();
                            //Toast.makeText(this, "Loaded " + otherCakeList.size() + " cakes", Toast.LENGTH_SHORT).show();

                        } else {
                            String message = response.optString("message", "No cakes found");
